...
public class Strat49 implements Strategie {
    ...
    private final List<Choice> choicesMade = new ArrayList<>();
    ...
    public int choix(Jeu j) {
        if (j.getBonzesRestants() == 3 && currentStep != 0) {
            if (choicesMade.size() > 0) {
                Choice ch = choicesMade.get(choicesMade.size() - 1);
                learner.update(ch.oldState, ch.action, ch.newState, -100);
            }
        }
    ...
    public boolean stop(Jeu j) {
        ...
        for (int[] bonze : bonzes) {
            ...
            if (maxs[bonze[0] - 2] == bonze[1]) {
                if (choicesMade.size() > 0) {
                    Choice ch = choicesMade.get(choicesMade.size() - 1);
                    learner.update(ch.oldState, ch.action, ch.newState, 10);
                }
                return true;
            }
        }
    ...
    public void end(boolean victoire) {
        for (Choice ch : choicesMade) {
            learner.update(ch.oldState, ch.action, ch.newState, victoire ? 10 : -10);
        }
        choicesMade.clear();
    }
}
